...
const MCP_TOOLS = [
  ...
  { name: "get_current_context", description: "调用本工具获取苹果乐园当前共享事实，再据此由 AI 做判断。返回的是事实快照，不代表苹果乐园已经替 AI 做出‘该提醒、允许、拒绝或批评’的主观决定。本工具不会执行用户任务、修改 Todo、主动改变 AppGate/Focus 决策或自动放行；底层模块可能在读取时清理已经按既定 TTL 自然过期的状态。Schedule 的 current_plan 只表示当前，today_remaining 只表示今天；next_plan 在 next_plan_search 标明的有界未来窗口内查找，none_within_window 不代表窗口之外确定没有计划。", inputSchema: obj({ device_id: str(DEFAULT_DEVICE), wait_seconds: int(5) }) },
